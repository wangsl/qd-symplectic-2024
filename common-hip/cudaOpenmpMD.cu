...
{ 
  ...
  for(int i_dev = 0; i_dev < n_devices(); i_dev++) {
    ...
    if(left >= 0) checkCudaErrors(hipDeviceEnablePeerAccess(left, 0));
    
    const int right = i_dev + 1;
    if(right < n_devices()) checkCudaErrors(hipDeviceEnablePeerAccess(right, 0));
  }
}
...
void CUDAOpenmpMD::disable_peer_to_peer_access() const
{
  ...
  for(int i_dev = 0; i_dev < n_devices(); i_dev++) {
    ...
    const int left = i_dev - 1;
    if(left >= 0) checkCudaErrors(hipDeviceDisablePeerAccess(left));
    
    const int right = i_dev + 1;
    if(right < n_devices()) checkCudaErrors(hipDeviceDisablePeerAccess(right));
  }
}
