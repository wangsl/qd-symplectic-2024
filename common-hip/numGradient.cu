...
void Num1ststGradient::copy_gradient_coefficients_to_device(const int n_points)
{
  ...
  size_t size = 0;
  checkCudaErrors(hipGetSymbolSize(&size, HIP_SYMBOL(gradient_coeffients_dev)));
  insist(size > n*sizeof(double));
  
  const double *coefs = coefficients(n_points);
  
  checkCudaErrors(hipMemcpyToSymbolAsync(HIP_SYMBOL(gradient_coeffients_dev), coefs, n*sizeof(double),
